export function installErrorGuard() {
  ...
  function isFullStoryError(ev: ErrorEvent | any, reason?: any) {
    try {
      const msg = (ev && ev.message) || (reason && reason.message) || (reason && String(reason)) || '';
      const stack = (ev && ev.error && ev.error.stack) || (reason && reason.stack) || '';
      if (msg.toLowerCase().includes('failed to fetch') && (stack.includes('fullstory') || msg.toLowerCase().includes('fullstory') || stack.includes('edge.fullstory.com'))) return true;
