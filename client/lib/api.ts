export async function apiGet<T>(path: string, fallback: T): Promise<T> {
  ...
  try {
    if (typeof fetch !== 'function') {
      ...
      console.warn('[apiGet] fetch not available, returning fallback for', path);
      return fallback;
    }
    ...
    // eslint-disable-next-line no-console
    if (e && e.name === 'AbortError') console.warn('[apiGet] request aborted (timeout)', path);
    else console.warn('[apiGet] fetch failed', path, e && e.message ? e.message : e);
    return fallback;
  } finally {
