...
function AuthView({ onSignedIn }: { onSignedIn: () => void }) {
  ...
  async function handleLogin(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!supabase) { setToast({ message: t("index.errors.supaNotConfigured"), type: "error" }); return; }
    ...
    else { setToast({ message: t("index.messages.loggedIn"), type: "success" }); onSignedIn(); }
  }
  ...
  async function handleSignup(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (password !== confirm) { setToast({ message: t("index.errors.passwordsDontMatch"), type: "error" }); return; }
    setLoading(true);
    const { error } = await supabase.auth.signUp({ email, password, options: { data: { full_name } } });
    setLoading(false);
    if (error) setToast({ message: error.message, type: "error" });
    else { setToast({ message: t("index.messages.accountCreated") , type: "success" }); setTab("login"); }
  }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-md animate-fade-in">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-[Pacifico] text-primary mb-2">{t("common.appName")}</h1>
          <p className="text-gray-600">{t("index.authSubtitle")}</p>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-8">
          <div className="flex justify-center mb-6">
            <div className="inline-flex rounded-full p-1 bg-gray-100">
              <button onClick={() => setTab("login")} className={`px-4 py-2 rounded-full font-medium ${tab === "login" ? "bg-white shadow-sm text-gray-800" : "text-gray-600"}`}>{t("index.tabs.login")}</button>
              <button onClick={() => setTab("signup")} className={`px-4 py-2 rounded-full font-medium ${tab === "signup" ? "bg-white shadow-sm text-gray-800" : "text-gray-600"}`}>{t("index.tabs.signup")}</button>
            </div>
          </div>
          ...
          {tab === "login" ? (
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t("index.fields.email")}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t("index.fields.password")}</label>
              ...
              <button type="submit" disabled={!supaAvailable || loading} className="w-full bg-primary text-white py-2 rounded-button font-medium disabled:opacity-50">{loading ? t("index.buttons.processing") : t("index.buttons.login")}</button>
              {!supaAvailable && (<p className="text-xs text-red-600">{t("index.errors.supaNotConfigured")}</p>)}
            </form>
          ) : (
            <form onSubmit={handleSignup} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t("index.fields.name")}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t("index.fields.confirmPassword")}</label>
              ...
              <button type="submit" disabled={!supaAvailable || loading} className="w-full bg-primary text-white py-2 rounded-button font-medium disabled:opacity-50">{loading ? t("index.buttons.processing") : t("index.buttons.signup")}</button>
            </form>
          )}
        </div>
        <div className="mt-4 text-center">
          <button className="text-primary text-sm" onClick={async () => {
            ...
            if (!email) return alert(t("index.errors.enterEmail"));
            if (!supabase) return alert(t("index.errors.supaConfigRequired"));
            await supabase.auth.resetPasswordForEmail(email, { redirectTo: window.location.origin });
            alert(t("index.messages.resetEmailSent"));
          }}>{t("index.forgotPassword")}</button>
        </div>
      </div>
    </div>
  );
...
function DashboardView({ userName }: { userName: string }) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setChat(prev => [...prev, { from: "bot", text: data.response || t("index.messages.troubleNow") }]);
    } catch (e) { setChat(prev => [...prev, { from: "bot", text: t("index.messages.troubleNow") }]); }
    finally { setSending(false); }
  }

  const localeMap: Record<string, string> = { en: "en-US", ko: "ko-KR" };
  const dateStr = new Date().toLocaleDateString(localeMap[locale] || "en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        <div className="bg-white rounded-lg shadow-sm p-6 animate-fade-in">
          <div className="flex flex-col md:flex-row md:items-center justify-between">
            <div className="flex items-center space-x-4 mb-4 md:mb-0">
              ...
              <div>
                <h2 className="text-xl font-bold">{t("dashboard.hello", { name: userName || "User" })}</h2>
                <p className="text-gray-600">{dateStr}</p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
              <div className="flex flex-col">
                <span className="text-sm text-gray-600 mb-1">{t("dashboard.feeling")}</span>
              ...
              <div className="flex flex-col">
                <span className="text-sm text-gray-600 mb-1">{t("dashboard.energy")}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-6 animate-fade-in" style={{ animationDelay: "0.05s" }}>
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center space-x-4">
                <h3 className="text-lg font-bold">{t("dashboard.schedule")}</h3>
                <div className="flex bg-gray-100 rounded-full p-1">
                  <button onClick={() => navigate('/calendar?view=day')} className="px-3 py-1 text-sm font-medium rounded-full bg-white shadow-sm text-gray-800 whitespace-nowrap">{t("dashboard.view.day")}</button>
                  <button onClick={() => navigate('/calendar?view=week')} className="px-3 py-1 text-sm font-medium rounded-full text-gray-600 hover:bg-white/50 whitespace-nowrap">{t("dashboard.view.week")}</button>
                  <button onClick={() => navigate('/calendar?view=month')} className="px-3 py-1 text-sm font-medium rounded-full text-gray-600 hover:bg-white/50 whitespace-nowrap">{t("dashboard.view.month")}</button>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <button onClick={() => navigate('/calendar')} className="px-3 py-1.5 text-sm font-medium hover:bg-gray-100 rounded-button">{t("dashboard.today")}</button>
                <button onClick={() => navigate('/calendar?create=1')} className="flex items-center justify-center space-x-1 bg-primary text-white px-3 py-1.5 font-medium rounded-button"><i className="ri-add-line" /><span className="whitespace-nowrap">{t("dashboard.addEvent")}</span></button>
              </div>
            </div>
            <div className="grid grid-cols-1 gap-2">
              {events.length === 0 && (<div className="text-sm text-gray-500 p-4 bg-gray-50 rounded-lg">{t("dashboard.noEvents")}</div>)}
              {events.map((ev) => {
                const start = ev.start_time ? new Date(ev.start_time) : null;
                const end = ev.end_time ? new Date(ev.end_time) : null;
                const timeLabel = start ? start.toLocaleTimeString(localeMap[locale] || "en-US", { hour: "numeric", minute: "2-digit" }) : "";
                return (
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 animate-fade-in" style={{ animationDelay: "0.1s" }}>
            <h3 className="text-lg font-bold mb-4">{t("dashboard.chatWithAssistant")}</h3>
            ...
            <div className="flex">
              <input value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder={t("dashboard.askAssistant")} className="flex-1 border border-gray-300 rounded-l-button py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
